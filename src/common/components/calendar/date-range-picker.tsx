...
const DateRangePicker = ({ defaultStartDate, defaultEndDate, onChange }: DateRangePickerProps) => {
  const today = dayjs()
  ...
  const handleDateClick = (clicked: Dayjs) => {
    if (selectingStart) {
      ...
      if (startDate) {
        ...
        } else if (clicked.isSame(startDate, "day")) {
          setStartDate(null)
  ...
  return (
    <div className="flex w-full flex-col gap-[22px]">
      <div className="mx-auto flex h-6 w-[160px] items-center justify-between">
        ...
        <p className="text-title6 leading-4 text-gray-800">
          {year}년 {month + 1}월
        </p>
      ...
      <div className="grid min-h-[220px] grid-cols-7 items-center justify-center gap-y-2">
